...
class SelectViewController2: UIViewController {
    
    let myStepLabel1: UILabel = UILabel(frame: CGRectMake(0,0,150,100))
    let myStepLabel2: UILabel = UILabel(frame: CGRectMake(0,0,270,100))
    ...
    override func viewDidLoad() {
        ...
        myStepper1.center = CGPointMake(self.view.frame.width/2, self.view.frame.height / 5 + 75)
        ...
        myStepper2.center = CGPointMake(self.view.frame.width/2, 3 * self.view.frame.height / 5 + 75)
        ...
        // 最小値, 最大値, 規定値の設定をする.
        myStepper1.minimumValue = 3
        myStepper1.maximumValue = 100
        myStepper1.value = 3
        
        myStepper2.minimumValue = 2
        myStepper2.maximumValue = 3
        myStepper2.value = 2
        ...
        myButtonNext.setTitle("決定", forState: .Normal)
        ...
        // Labelを作成する.
        myStepLabel1.backgroundColor = UIColor.greenColor()
        myStepLabel1.layer.masksToBounds = true
        myStepLabel1.layer.cornerRadius = 25.0
        ...
        myStepLabel1.layer.position = CGPoint(x: self.view.bounds.width/2,y: self.view.frame.height / 5)
        myStepLabel1.text = "\(Int(myStepper1.value))"+"人を"
        
        myStepLabel2.backgroundColor = UIColor.greenColor()
        myStepLabel2.layer.masksToBounds = true
        myStepLabel2.layer.cornerRadius = 25.0
        myStepLabel2.text = "\(Int(myStepper2.value))"+"チームに分ける"
        ...
        myStepLabel2.layer.position = CGPoint(x: self.view.bounds.width/2,y: 3 * self.view.frame.height / 5 )
        //myStepLabel2.text = "\(Int(myStepper2.value))"+"チームに分ける"
    ...
    func stepperOneChanged1(stepper: UIStepper){
        ...
        if( myStepper2.value > myStepper1.value){
            myStepper2.value = myStepper1.value
            myStepLabel2.text = "\(Int(myStepper2.value))"+"チームに分ける"
        }
        myStepper2.maximumValue = myStepper1.value
    ...
    func onClickMyButton(sender: UIButton){
        ...
        // 遷移するViewを定義する.
        let myResultViewController: UIViewController = ResultViewController2()
        
        //計算用ModelController定義する
        let myModel: Model = Model()
        var elementarray:[UInt32] = myModel.getrandom(x, yi: y)
        var element:UInt32 = y
